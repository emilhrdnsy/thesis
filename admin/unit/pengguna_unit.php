                        ...
                        else if($ddatagrid['blokir']=='A')
                        {
                          ...
                          <tr>
                            ...
                            <td style=text-align:center;vertical-align:middle>
                              <a href=?unit=pengguna_unit&act=update&id_user=$ddatagrid[id_user] class='btn btn-sm btn-warning glyphicon glyphicon-pencil' ></a> 
                              <a href=# class='btn btn-sm btn-danger glyphicon glyphicon-trash' onclick='confirm($ddatagrid[id_user])'></a>  
                            </td>                
                          </tr>
                          ";
                          $no++;
  ...
  function confirm(id_user) {
    swal.fire({
      ...
      if (result.value) {
        ...
        setTimeout(function () {
          window.location.href = "?unit=pengguna_unit&act=delete&id_user=" + id_user
        }, 1000);
      } else {
        Swal.fire({
          title: 'Dibatalkan',
      ...
      <div class="row">
        <div class="col-xs-12">
          ...
          <?php
            ...
            $qupdate = "SELECT max(id_user) as maxKode FROM t_user";

            $rupdate = mysqli_query($mysqli, $qupdate);
            $dupdate = mysqli_fetch_assoc($rupdate);
            $kode_user = $dupdate['maxKode'];
            $no_urut = $kode_user + 1;
            $char = "USER-S";
            $newID = $char.sprintf("%01s",$no_urut);
          ?>

          <form class="form-horizontal" method="post" action="?unit=pengguna_unit&act=inputact"
            enctype="multipart/form-data">

            <div class="form-group">
              <label class="col-sm-3 control-label no-padding-right" for="nama">Kode Pengguna</label>
              <div class="col-sm-9">
                <input class="col-xs-10 col-sm-5" type="text" name="kode_user" id="kode_user"
...
<?php
  ...
  case "inputact":
    ...
    $qinput = " INSERT INTO t_user (nama_pengguna, nama, kata_sandi, kode_user)
              VALUES ('$nama_pengguna', '$nama','$kata_sandi', '$kode_user') ";
  ...
  case "update":
    ...
    $qupdate = "SELECT * FROM t_user WHERE id_user = '$id_user'";
    $rupdate = mysqli_query($mysqli, $qupdate);
    $dupdate = mysqli_fetch_assoc($rupdate);
?>
    ...
    <div class="page-content">
      ...
      <div class="row">
        <div class="col-xs-12">
          ...
          <form class="form-horizontal" method="post" action="?unit=pengguna_unit&act=updateact"
            enctype="multipart/form-data">
            <div class="form-group">
              <label class="col-sm-3 control-label no-padding-right" for="kode_user">Kode Pengguna</label>
              <div class="col-sm-9">
                <input class="col-xs-10 col-sm-5" type="text" name="kode_user" id="kode_user" readonly="readonly"
                  value="<?php echo $dupdate['kode_user'] ?>" required />
              </div>
            </div>
            <div class="form-group">
              <label class="col-sm-3 control-label no-padding-right" for="nama">Nama</label>
...
<?php
  ...
  case "updateact":
    ...
    $qupdate = "";
    if($kata_sandi == "") {
      $qupdate = " UPDATE t_user SET nama_pengguna = '$nama_pengguna', nama = '$nama'
                  WHERE kode_user = '$kode_user'
      ";            
    }
    else {
      $qupdate = " UPDATE t_user SET nama_pengguna = '$nama_pengguna', nama = '$nama', kata_sandi = '$kata_sandi'    
                  WHERE kode_user = '$kode_user'
      ";                        
    }
    if($qupdate)
  ...
  case "aktif":
    $id_user = $_GET['id_user'];
    $blokir = $_POST['blokir'];
    $qupdate = " UPDATE t_user SET blokir = 'N', bataslogin = '0' 
                WHERE id_user = '$id_user' ";
    $rupdate = mysqli_query($mysqli,$qupdate);
    header("location:?unit=pengguna_unit&act=datagrid");
  break;
  ...
  case "blokir":
    $id_user = $_GET['id_user'];
    $blokir = $_POST['blokir'];
    $qupdate = " UPDATE t_user SET blokir = 'Y'  WHERE
          id_user = '$id_user'  ";
    $rupdate = mysqli_query($mysqli,$qupdate);
    header("location:?unit=pengguna_unit&act=datagrid");
  break;
